...
trait HilSuite extends TestplanSuite {
  ...
  def hwScenario(name : String, variant : String = "", needs : Set[HilSide.Value] = HilSide.Both)
    ...
    testpoint(name, variant) {
      val label = if (variant.isEmpty) name else s"$name ($variant)"
      HilProgress.watch(label)                             // takze otwieranie portow
      try scenario(name, variant, label, needs, body)
      finally HilProgress.unwatch()
    }

  private def scenario(name : String, variant : String, label : String, needs : Set[HilSide.Value],
                       body : HilBench => Unit) : Unit = {
    ...
    hb.logTo(Some(dir))
    HilProgress(s"$label: start, log ruchu ${dir.getAbsolutePath}")
    try body(hb)
    finally {
      HilProgress(s"$label: koniec")
      hb.logTo(None)
